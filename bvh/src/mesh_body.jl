...
function MeshBody(fname::String;scale=1.0,boundary=true,thk=0f0,T=Float32)
    ...
    return MeshBody(mesh,mesh,srf_id,(x,t)->x,bbox,T(scale),T(thk/2),boundary)
end
...
#updated to not work recursivly update function is applied to initial mesh 
function WaterLily.update!(body::MeshBody;update::Function=(x)->x)
    ...
    points =Point3f[]
    for (i,pnt) in enumerate(body.mesh_0.position)
        push!(points, Point3f(update(SA[pnt.data...])))
    end
    body.mesh=GeometryBasics.Mesh(points,GeometryBasics.faces(body.mesh_0))
    bbox = Rect(points)
    body.bbox = Rect(bbox.origin.-max(4,2body.half_thk),bbox.widths.+max(8,4body.half_thk))
end


#update mesh0 to update mesh0 for initial movement such as moving to center updates both mesh_0 and mesh
function update_mesh0!(body::MeshBody;update::Function=(x)->x)
    @assert((hasmethod(update,Tuple{SVector}) && typeof(update(SVector{3}(zeros(3))))==typeof(SVector{3}(zeros(3)))),
    "`update` must have a method of type ::SVector{3}->::SVector{3}")
    points =Point3f[]
    for (i,pnt) in enumerate(body.mesh_0.position)
    ...
    body.mesh_0=GeometryBasics.Mesh(points,GeometryBasics.faces(body.mesh_0))
    body.mesh=GeometryBasics.Mesh(points,GeometryBasics.faces(body.mesh_0))
    bbox = Rect(points)
    body.bbox = Rect(bbox.origin.-max(4,2body.half_thk),bbox.widths.+max(8,4body.half_thk))
end
...
"""
#_fast remove for more accuracy
...
function get_p(tri::GeometryBasics.Ngon{3},p::AbstractArray{T,3},δ) where T
    ...
    ar.*n.*interp(c.+1.5 .+ δ.*n, p)-ar.*n.*interp(c.+1.5 .- δ.*n,p)
end
#interp should be difference between + and minus delta

#code up viscous force 

# validate with very simple case ~50 elements 
