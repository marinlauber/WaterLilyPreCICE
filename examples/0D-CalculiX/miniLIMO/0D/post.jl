...
# load the file
data = CSV.read("/home/marin/Workspace/WaterLilyPreCICE/examples/0D-CalculiX/miniLIMO/0D/sphere_output.csv", DataFrame; delim = ',')
...
data_conv_limo = @views(data[idx_last, :])
...
time = data_conv_limo.time
deltat = time[2]-time[1]
tmax = maximum(time)
p2 = plot(time, data_conv_limo.VLV_0D, label="VLV 0D", alpha=0.5, c=:orange, xlims=(0,tmax))
plot!(p2, time, data_conv_limo.PAO_0D, label="PAO 0D", alpha=0.5, c=:blue)

p1 = plot(time, data_conv_limo.Pfill_0D, label="Pfill 0D", c=:cyan)
plot!(p1, time, data_conv_limo.PACT_3D, label="Pact 3D", c=:magenta)
plot!(p1, time, data_conv_limo.PLV_0D, label="PLV 0D", c=:green)
plot!(p1, time, data_conv_limo.PLV_3D, c=:red, label="PLV 3D", ylabel="Pressure (mmHg)", alpha=0.5, xlims=(0,tmax), ylims=(0,120))
plot!(p1, time, data_conv_limo.PAO_0D, label="PAO 0D", alpha=0.5, c=:blue)
plot(p2, time, data_conv_limo.VLV_3D, c=:black, label=:none, ylabel="Volume (ml)", alpha=0.5, xlims=(0,tmax))

p6 = plot(data_conv_limo.VLV_3D, data_conv_limo.PLV_3D, label="3D", ylabel="PLV (mmHg)", xlabel="VLV (ml)", xlims=(0,160), ylims=(0,100))
plot!(p6, data_conv_limo.VLV_0D, data_conv_limo.PLV_0D, label="0D", c=:orange)

# flow rates
p5 = plot(time, -data_conv_limo.QAO_0D./12, label="QAO 0D",
          xlabel="time (s)", ylabel="Flow rate (ml/s)",xlims=(0,tmax))
plot!(p5, time, data_conv_limo.QMV_0D./12, label="QMV 0D")
...
for i in 1:maximum(Int,data.timestep)
    ...
    # 0D model ouptu
    plot!(p2, xs, data.VLV_0D[idx_ts], label=:none, c=:orange, lw=2)
