...
let
    # iteration storage
    storage_step = []
    ω⁰ = 2.0; ωᵏ = 0.0                  # relaxation factor for the pressure
    r⁰ = 0.f0; rᵏ = 0.f0
    ...
    # old force vecotr to bypass convergence
    f⁰ = copy(interface.forces)
    ...
    while PreCICE.isCouplingOngoing()
        ...
        Pact = 196.0 * ϕᵢ(sum(interface.Δt)/10;tC=0.1,tR=0.4,TC=0.3,TR=0.3)
        ...
        println(" Ventricular pressure: $(round(Plv_k,digits=3)) mmHg, actuation pressure: $(round(Pact,digits=3)) mmHg, ",
                "interface volume: $(round(vol_3D,digits=3)) ml, target volume: $(round(vlv_0D,digits=3)) ml")
        ...
        # manual check if we are not "converged in terms of volume
        if abs(rᵏ) < 1e-4
            # interface.forces .= 1.00001f0.*f⁰ # this means convergence
            println(" Forcing convergence for volume residual: $(round(abs(rᵏ/vlv_0D),digits=6))")
        end

        # write data to the other participant, advance coupling
        writeData!(interface)
        f⁰ .= interface.forces # for next iteration or next time step
